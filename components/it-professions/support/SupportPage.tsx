  ...
  return (
    <div className="font-poppins bg-gray-50 min-h-screen text-black">
      ...
      <section className="max-w-7xl mx-auto px-6 py-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {[
          { icon: "fas fa-question-circle", title: "FAQs", text: "Find quick answers to common questions.", link: "#faqs" },
          { icon: "fas fa-envelope", title: "Email Support", text: "We'll respond within 24 hours.", link: "mailto:support@dousoft.com" },
          { icon: "fas fa-phone-alt", title: "Call Us", text: "Speak directly with support.", link: "tel:+01523456789" },
          { icon: "fas fa-comments", title: "Live Chat", text: "Chat with a representative in real-time.", link: "#" },
          { icon: "fas fa-book", title: "Documentation", text: "Access guides and tutorials.", link: "#" },
          { icon: "fas fa-users", title: "Community Forum", text: "Connect with other learners.", link: "#" },
        ].map((item, idx) => (
          <div key={idx} className="bg-white rounded-2xl shadow-md p-6 flex flex-col gap-4 items-start hover:shadow-xl transition">
            <i className={`${item.icon} text-3xl text-orange-600`}></i>
            <h3 className="text-xl font-semibold">{item.title}</h3>
            <p className="text-gray-600 text-sm">{item.text}</p>
            <a href={item.link} className="text-orange-600 font-medium hover:underline flex items-center gap-1">
        ...
        <div className="flex flex-col gap-4">
          {faqs.map((faq, idx) => (
            <div key={idx} className="bg-white rounded-2xl shadow-md overflow-hidden">
              ...
              >
                {faq.question} <ChevronDown className={`transition-transform ${faqActive === idx ? "rotate-180" : ""}`} />
              </button>
              {faqActive === idx && (
                <div className="px-6 py-4 text-gray-600 border-t border-gray-200">{faq.answer}</div>
              )}
            </div>
          ))}
        </div>
      ...
      <section className="bg-white py-16">
        <div className="max-w-3xl mx-auto px-6">
          ...
          <form className="flex flex-col gap-4">
            <input type="text" placeholder="Full Name" className="w-full px-4 py-3 border rounded-xl focus:outline-none" required />
            <input type="email" placeholder="Email Address" className="w-full px-4 py-3 border rounded-xl focus:outline-none" required />
            ...
            <textarea placeholder="Your Message" className="w-full px-4 py-3 border rounded-xl focus:outline-none" rows={6} required></textarea>
            <button className="w-full py-3 bg-orange-600 text-white rounded-xl hover:bg-orange-500 transition">Send Message</button>
          </form>
        </div>
      </section>
